...
#     print(f"DELETE route hit for user_id: {user_id}")  # Log entry

#     try:
#         object_id = ObjectId(user_id)  # Convert to ObjectId
#         print(f"Converted user_id to ObjectId: {object_id}") # Log Conversion

#         result = users_collection.delete_one({'_id': object_id}) #pass object_id
#         print(f"Delete result: {result.deleted_count}") #Log result
...
#         result = cars_collection.delete_one({'_id': ObjectId(data['carId'])})
#         if result.deleted_count == 1:
#             return jsonify({'message': 'Car deleted successfully by admin'}), 200
#         else:
#             return jsonify({'message': 'Car not found'}), 404
#     except Exception as e:
#         return jsonify({'message': 'Failed to delete car', 'error': str(e)}), 500
...
#     users_data = {month: 0 for month in months}
#     cars_data = {month: 0 for month in months}
#     trips_data = {month: 0 for month in months}
#     bookings_data = {month: 0 for month in months}
...
#         users_data[item['month']] = item['count']

#     # Aggregation pipeline for cars
#     cars_pipeline = [
#         {
#             '$match': {
#                 'created_at': {
...
#     for item in cars_result:
#         cars_data[item['month']] = item['count']

#     # Aggregation pipeline for trips
#     trips_pipeline = [
#         {
#             '$match': {
#                 'created_at': {
...
#     for item in trips_result:
#         trips_data[item['month']] = item['count']
...
#     for item in bookings_result:
#         bookings_data[item['month']] = item['count']

#     for month in months:
#         chart_data.append({'month': month, 'data_type': 'users', 'count': users_data[month]})
#         chart_data.append({'month': month, 'data_type': 'cars', 'count': cars_data[month]})
#         chart_data.append({'month': month, 'data_type': 'trips', 'count': trips_data[month]})
#         chart_data.append({'month': month, 'data_type': 'bookings', 'count': bookings_data[month]})
...
#     try:
#         # Aggregate trips by car model and calculate total amount
#         pipeline = [
#             {
#                 '$group': {
#                     '_id': '$carModel',
